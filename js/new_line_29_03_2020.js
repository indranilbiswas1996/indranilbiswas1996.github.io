...
function create_paper(){
	//console.log(instructions[current_graph_id].container+width+" "+height);
	paper = new Raphael(document.getElementById(instructions[current_graph_id].container), width, height);	
...
function icon_design(){
	...
	for(var i = 0; i < path_details[current_graph_id].length; i++){
		...
		var text = elems["paper_"+current_graph_id].text(width - x, paddingy[current_graph_id], path_details[current_graph_id][(path_details[current_graph_id].length-1)-i].name).attr({
			'font-weight' : 'bold',
			'text-anchor': 'start',
			'font-size' : '13px',
			fill : path_details[current_graph_id][(path_details[current_graph_id].length-1)-i].color,
		});
		var circle = elems["paper_"+current_graph_id].circle(width- x - (.3*paddingx[current_graph_id]) , paddingy[current_graph_id], 5).attr({
			fill : path_details[current_graph_id][(path_details[current_graph_id].length-1)-i].color,
			stroke : path_details[current_graph_id][(path_details[current_graph_id].length-1)-i].color,
		});
		var rect = elems["paper_"+current_graph_id].rect(width- x - (.3*paddingx[current_graph_id]) - 5, paddingy[current_graph_id] - 7.5, (path_details[current_graph_id][(path_details[current_graph_id].length-1)-i].name).toString().length*7 + 25 , 15).attr({
			fill : 'transparent',
			opacity : 0,
			cursor : 'pointer',
			//stroke : 'transparent',
...
function x_axis_title(){
	elems["paper_"+current_graph_id].text(width/2, height-paddingy[current_graph_id], instructions[current_graph_id].x_axis_name);
	x_axis_title_flag = 1;
}
...
function y_axis_title(){
	elems["paper_"+current_graph_id].text(paddingx[current_graph_id], height/2, instructions[current_graph_id].y_axis_name).attr({
		transform : "r270",
	});
	y_axis_title_flag = 1;
}
...
function x_axis(){
	if(0 <= max_values[current_graph_id] && 0 >= min_values[current_graph_id]){
		elems["paper_"+current_graph_id].path("M " + (2*paddingx[current_graph_id]) + " " + ( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(0 - min_values[current_graph_id]))/maxs[current_graph_id]) + "l "+ (width - (4*paddingx[current_graph_id])) + " 0").attr({
			stroke : instructions[current_graph_id].axis_color || "#9d9d9d",
		});
	}else{
		elems["paper_"+current_graph_id].path("M " + (2*paddingx[current_graph_id]) + " " + ( (height-(2*paddingy[current_graph_id])) ) + "l "+ (width - (4*paddingx[current_graph_id])) + " 0").attr({
...
var y_axis_flag = 0;
function y_axis(){
	elems["paper_"+current_graph_id].path("M " + (2*paddingx[current_graph_id]) + " " + (2*paddingy[current_graph_id]) +" l  0 " + (height-(4*paddingy[current_graph_id])) ).attr({
		stroke : instructions[current_graph_id].axis_color || "#9d9d9d",
	});
	y_axis_flag = 1;
}
...
function x_axis_division(){
	...
	for(var i = 0; j< datas[current_graph_id].length; i++){	
		elems["paper_"+current_graph_id].path("M " + ((2*paddingx[current_graph_id]) + ((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))*j) + " "+ (height - (2*paddingy[current_graph_id])) + " l 0 " + (-(height-(4*paddingy[current_graph_id]))) ).attr({
			stroke : "#ececec",
			opacity : .8,
		});
...
function x_axis_label(){
	var j = 0;		
	for(var i = 0; j< datas[current_graph_id].length; i++){		
		elems["paper_"+current_graph_id].path( "M " + ((2*paddingx[current_graph_id]) + ((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))*j) + " "+ (height - (2*paddingy[current_graph_id])) + " l 0 3" ).attr({
			stroke : instructions[current_graph_id].axis_color || "#9d9d9d",
		});
		if(x_division_nos[current_graph_id]>15){
			elems["paper_"+current_graph_id].text(((2*paddingx[current_graph_id]) + ((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))*j) , (height - (1.5*paddingy[current_graph_id])), datas[current_graph_id][j].month).attr({
...
function y_axis_division(){
	...
	for(var i = 0 ; i<y_division_nos[current_graph_id] ; i++){		
		elems["paper_"+current_graph_id].path( "M "+ 2*paddingx[current_graph_id] +" " + ( (height-(2*paddingy[current_graph_id]))-((height-(4*paddingy[current_graph_id]))/(y_division_nos[current_graph_id]-1))*i ) + " l "+  (width-(4*paddingx[current_graph_id])) +" 0").attr({
			stroke : "#ececec",
			opacity : .8,
		});
	}
...
function y_axis_label(){
	for(var i = 0 ; i<y_division_nos[current_graph_id] ; i++){
		elems["paper_"+current_graph_id].path( "M "+ 2*paddingx[current_graph_id] +" " + ( (height-(2*paddingy[current_graph_id]))-((height-(4*paddingy[current_graph_id]))/(y_division_nos[current_graph_id]-1))*i ) + " l -3 0").attr({
			stroke : instructions[current_graph_id].axis_color || "#9d9d9d",
		});
		elems["paper_"+current_graph_id].text(2*paddingx[current_graph_id] - 5, (height-(2*paddingy[current_graph_id]))-((height-(4*paddingy[current_graph_id]))/(y_division_nos[current_graph_id]-1))*i, ((maxs[current_graph_id]/(y_division_nos[current_graph_id]-1))*i)+min_values[current_graph_id]).attr({
			fill : instructions[current_graph_id].axis_color || "#9d9d9d",
			'text-anchor' : "end",
		});		
	}
	...
	for(var i = 0; i < parseInt(instructions[current_graph_id].path_no); i++){
		for(var x = 0; x<path_details[current_graph_id][i].type.length; x++){
			if(path_details[current_graph_id][i].type[x] == "line_path"){
				p = "M "+ (2*paddingx[current_graph_id]) + " "+ ( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(0 - min_values[current_graph_id]))/maxs[current_graph_id]) + " ";
				for(var j = 1; j < datas[current_graph_id].length ; j++){
					p += " l "+ (width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1) + " "+ ((height-(4*paddingy[current_graph_id]))*0)/maxs[current_graph_id] + "";
				}
				...
				p = "M "+ (2*paddingx[current_graph_id]) + " "+ ( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(Object.values(datas[current_graph_id][0].value)[i] - min_values[current_graph_id]))/maxs[current_graph_id]) + " ";
				for(var j = 1; j < datas[current_graph_id].length ; j++){
					p += " l "+ (width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1) + " "+ ((height-(4*paddingy[current_graph_id]))*(Object.values(datas[current_graph_id][j-1].value)[i] - Object.values(datas[current_graph_id][j].value)[i]))/maxs[current_graph_id] + "";
				}
	...
	for(var i = 0; i < parseInt(instructions[current_graph_id].path_no); i++){
		for(var x = 0; x<path_details[current_graph_id][i].type.length; x++){
			if(path_details[current_graph_id][i].type[x] == "round_path"){
				p = "M "+ (2*paddingx[current_graph_id]) + " "+ ((height - (2*paddingy[current_graph_id])) - ((0   - min_values[current_graph_id])*((height-(4*paddingy[current_graph_id]))/maxs[current_graph_id]))) + " R ";
				for(var j = 1; j < datas[current_graph_id].length ; j++){
					p += ", "+ ((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))) + ", "+ ((height - (2*paddingy[current_graph_id])) - ((0 - min_values[current_graph_id])*((height-(4*paddingy[current_graph_id]))/maxs[current_graph_id]))) + " ";
				}
				var path1 = elems["paper_"+current_graph_id].path(p).attr({
					'stroke-width' : path_details[current_graph_id][i].stroke[x],
					stroke : path_details[current_graph_id][i].color,
				...
				p = "M "+ (2*paddingx[current_graph_id]) + " "+ ((height - (2*paddingy[current_graph_id])) - ((Object.values(datas[current_graph_id][0].value)[i]   - min_values[current_graph_id])*((height-(4*paddingy[current_graph_id]))/maxs[current_graph_id]))) + " R ";
				for(var j = 1; j < datas[current_graph_id].length ; j++){
					p += ", "+ ((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))) + ", "+ ((height - (2*paddingy[current_graph_id])) - ((Object.values(datas[current_graph_id][j].value)[i]  - min_values[current_graph_id])*((height-(4*paddingy[current_graph_id]))/maxs[current_graph_id]))) + " ";
				}
	...
	for(var i = 0; i < parseInt(instructions[current_graph_id].path_no); i++){
		for(var x = 0; x<path_details[current_graph_id][i].type.length; x++){
			if(path_details[current_graph_id][i].type[x] == "stepline_path"){
				p = "M "+ (2*paddingx[current_graph_id]) + " "+ ( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(0  - min_values[current_graph_id]))/maxs[current_graph_id]) + " ";
				for(var j = 1; j < datas[current_graph_id].length ; j++){
					p += " l "+ (width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1) + " "+ 0 + "";
					p += " l "+ 0 + " "+ ((height-(4*paddingy[current_graph_id]))*0)/maxs[current_graph_id] + "";
				}
				var path1 = elems["paper_"+current_graph_id].path(p).attr({
					'stroke-width' : path_details[current_graph_id][i].stroke[x],
					stroke : path_details[current_graph_id][i].color,
				...
				for(var j = 1; j < datas[current_graph_id].length ; j++){
					p += " l "+ (width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1) + " "+ 0 + "";
					p += " l "+ 0 + " "+ ((height-(4*paddingy[current_graph_id]))*(Object.values(datas[current_graph_id][j-1].value)[i] - Object.values(datas[current_graph_id][j].value)[i]))/maxs[current_graph_id] + "";
				}
	...
	for(var i = 0; i < parseInt(instructions[current_graph_id].path_no); i++){
		if(path_details[current_graph_id][i].circle == true){
			for(var j = 0; j < datas[current_graph_id].length ; j++){
				var circle1 = elems["paper_"+current_graph_id].circle( ((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))), (height - (2*paddingy[current_graph_id])) - ((0  - min_values[current_graph_id])*((height-(4*paddingy[current_graph_id]))/maxs[current_graph_id])) ,path_details[current_graph_id][i].circle_zoom_out).attr({
					opacity : 1,
					fill : path_details[current_graph_id][i].color,
					stroke : path_details[current_graph_id][i].color,
				});
				circle1.animate({
					cx : ((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))),
					cy : (height - (2*paddingy[current_graph_id])) - ((Object.values(datas[current_graph_id][j].value)[i] - min_values[current_graph_id])*((height-(4*paddingy[current_graph_id]))/maxs[current_graph_id])),
	...
	for(var i = 0; i < parseInt(instructions[current_graph_id].path_no); i++){
		for(var x = 0; x<path_details[current_graph_id][i].type.length; x++){
			if(path_details[current_graph_id][i].type[x] == "line_area"){
				...
				p += " l "+ 0 + " "+ ((height-(4*paddingy[current_graph_id]))*(0 - 0))/maxs[current_graph_id] + "l -" + (width - (4*paddingx[current_graph_id])) + " 0z";
				var path1 = elems["paper_"+current_graph_id].path(p).attr({
				...
				p += " l "+ 0 + " "+ ((height-(4*paddingy[current_graph_id]))*(Object.values(datas[current_graph_id][j-1].value)[i] - 0))/maxs[current_graph_id] + "l -" + (width - (4*paddingx[current_graph_id])) + " 0z";
	...
	for(var i = 0; i < parseInt(instructions[current_graph_id].path_no); i++){	
		for(var x = 0; x<path_details[current_graph_id][i].type.length; x++){	
			if(path_details[current_graph_id][i].type[x] == "round_area"){
				...
				p+= ", "+ ((2*paddingx[current_graph_id])+(datas[current_graph_id].length-1)*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))) + ", "+ ((height - (2*paddingy[current_graph_id])) - ((0  - min_values[current_graph_id])*((height-(4*paddingy[current_graph_id]))/maxs[current_graph_id]))) + " z";
				var path1 = elems["paper_"+current_graph_id].path(p).attr({
					'stroke-width' : path_details[current_graph_id][i].stroke[x],
					stroke : path_details[current_graph_id][i].color,
					opacity : .1,
	...
	for(var i = 0; i < parseInt(instructions[current_graph_id].path_no); i++){
		if(path_details[current_graph_id][i].label == true){
			...
			for(var k = 0; j< datas[current_graph_id].length; k++){
				var rect1 = elems["paper_"+current_graph_id].rect( ((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))) - ((((Object.values(datas[current_graph_id][j].value)[i]+"").length*7) + 10)/2) ,(height - (2*paddingy[current_graph_id]))- ((0  - min_values[current_graph_id])*((height-(4*paddingy[current_graph_id]))/maxs[current_graph_id])), ((Object.values(datas[current_graph_id][j].value)[i]+"").length*7) + 10 , 16, 2).attr({
					//opacity : 0,
					fill : path_details[current_graph_id][i].color,		
					stroke : "#fff",		
				});
				rect1.animate({
					opacity : 1,
					y : (height - (2*paddingy[current_graph_id])) - ((Object.values(datas[current_graph_id][j].value)[i]  - min_values[current_graph_id])*((height-(4*paddingy[current_graph_id]))/maxs[current_graph_id])) - 8,
				}, 400 + (150*i), "<>" );
				elems["label_rect_"+current_graph_id+"_"+i+""+k] = rect1;
				var text1 = elems["paper_"+current_graph_id].text( ((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))), (height - (2*paddingy[current_graph_id])) - ((0  - min_values[current_graph_id])*((height-(4*paddingy[current_graph_id]))/maxs[current_graph_id])) ,Object.values(datas[current_graph_id][j].value)[i]).attr({
					//opacity : 0,
					fill : "#fff",//path_detail[i].color,
				});
				text1.animate({
					opacity : 1,
					x : ((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))),
					y : (height - (2*paddingy[current_graph_id])) - ((Object.values(datas[current_graph_id][j].value)[i]  - min_values[current_graph_id])*((height-(4*paddingy[current_graph_id]))/maxs[current_graph_id])),
						...
						for(var j = 0; j < datas[current_graph_id].length; j++){
							if(datas[current_graph_id][j].month == instructions[current_graph_id].annotations.xaxis[i].x){
								...
								if(j < datas[current_graph_id].length/2){
									rect_x = ((2*paddingx[current_graph_id]) + ((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))*j);
									...
									rect_x = ((2*paddingx[current_graph_id]) + ((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))*j)-20;
									rect_y = 2*paddingy[current_graph_id] +10;//+ (j*5);
									text_x = rect_x + 10;
									text_y = rect_y + ((instructions[current_graph_id].annotations.xaxis[i].label.text.length*7)+20)/2;
								}
					...
					if(instructions[current_graph_id].annotations.xaxis[i].x1){
						if(instructions[current_graph_id].annotations.xaxis[i].x2){							
							...
							if(x1 >=0 && x2 >= 0){
								...
								area_rect_x = ((2*paddingx[current_graph_id]) + ((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))*x1);
								area_rect_y = (2*paddingy[current_graph_id]);
								area_rect_width = ((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))*(x2-x1);
								area_rect_height = (height-(4*paddingy[current_graph_id]));
								rect_x = ((2*paddingx[current_graph_id]) + ((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))*x1);
								rect_y = height - (4*paddingy[current_graph_id]) -30 - (x_area_annotation_count*30);
								text_x = rect_x + 10;
								text_y = rect_y + ((instructions[current_graph_id].annotations.xaxis[i].label.text.length*7)+20)/2;
							}
							if(x1 == -1 && x2 >=0){
								...
								area_rect_y = (2*paddingy[current_graph_id]);
								area_rect_width = ((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))*x2;
								area_rect_height = (height-(4*paddingy[current_graph_id]));
								if(x2==0){
									rect_x = ((2*paddingx[current_graph_id]) + ((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))*0) - 5000;
								}else{
									rect_x = ((2*paddingx[current_graph_id]) + ((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))*0);
								}
								rect_y = height - (4*paddingy[current_graph_id]) -30 - (x_area_annotation_count*30);
								text_x = rect_x + 10;
								text_y = rect_y + ((instructions[current_graph_id].annotations.xaxis[i].label.text.length*7)+20)/2;
							...
							if(x1 >=0 && x2 == -1){
								area_rect_x = ((2*paddingx[current_graph_id]) + ((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))*x1);
								area_rect_y = (2*paddingy[current_graph_id]);
								area_rect_width = (width-(4*paddingx[current_graph_id])) - ((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))*x1;
								area_rect_height = (height-(4*paddingy[current_graph_id]));								
								if(datas[current_graph_id].length == x1+1){
									rect_x = ((2*paddingx[current_graph_id]) + ((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))*x1) +5000;									
								}else{
									rect_x = ((2*paddingx[current_graph_id]) + ((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))*x1);
								}
								rect_y = height - (4*paddingy[current_graph_id]) -30 - (x_area_annotation_count*30);
							...
							if(inner_flag == 2){
								area_rect_x = ((2*paddingx[current_graph_id]));
								area_rect_y = (2*paddingy[current_graph_id]);
								area_rect_width = (width-(4*paddingx[current_graph_id]));
								area_rect_height = (height-(4*paddingy[current_graph_id]));
								rect_x = ((2*paddingx[current_graph_id]) + ((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))*0);
								rect_y = height - (4*paddingy[current_graph_id]) -30 - (x_area_annotation_count*30);
								text_x = rect_x + 10;
				...
				for(var i = 0; i< instructions[current_graph_id].annotations.yaxis.length; i++){
					...
					if(instructions[current_graph_id].annotations.yaxis[i].y || instructions[current_graph_id].annotations.yaxis[i].y == 0){
						if(instructions[current_graph_id].annotations.yaxis[i].y < max_values[current_graph_id] && instructions[current_graph_id].annotations.yaxis[i].y > min_values[current_graph_id]){
							elems["paper_"+current_graph_id].path("M "+ 2*paddingx[current_graph_id] +" " + ( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(instructions[current_graph_id].annotations.yaxis[i].y - min_values[current_graph_id]))/maxs[current_graph_id]) + " l "+  (width-(4*paddingx[current_graph_id])) +" 0").attr({
							...
							if((instructions[current_graph_id].annotations.yaxis[i].y - min_values[current_graph_id]) > maxs[current_graph_id]/2){
								elems["paper_"+current_graph_id].rect((2*paddingx[current_graph_id]) + (i*50+50), (( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(instructions[current_graph_id].annotations.yaxis[i].y - min_values[current_graph_id]))/maxs[current_graph_id])), (instructions[current_graph_id].annotations.yaxis[i].label.text.length*7)+20, 20).attr({
								...
								elems["paper_"+current_graph_id].text((2*paddingx[current_graph_id]) + (i*50+50) + ((instructions[current_graph_id].annotations.yaxis[i].label.text.length*7)+20)/2, (( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(instructions[current_graph_id].annotations.yaxis[i].y - min_values[current_graph_id]))/maxs[current_graph_id]))+10, (instructions[current_graph_id].annotations.yaxis[i].label.text)).attr({
									fill : instructions[current_graph_id].annotations.yaxis[i].label.style.color,
								});
							}else{
								elems["paper_"+current_graph_id].rect((2*paddingx[current_graph_id]) + (i*50+50), (( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(instructions[current_graph_id].annotations.yaxis[i].y - min_values[current_graph_id]))/maxs[current_graph_id]))-20, (instructions[current_graph_id].annotations.yaxis[i].label.text.length*7)+20, 20).attr({
									fill : instructions[current_graph_id].annotations.yaxis[i].label.style.background,
									stroke : instructions[current_graph_id].annotations.yaxis[i].label.borderColor,
								});
								elems["paper_"+current_graph_id].text((2*paddingx[current_graph_id]) + (i*50+50) + ((instructions[current_graph_id].annotations.yaxis[i].label.text.length*7)+20)/2, (( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(instructions[current_graph_id].annotations.yaxis[i].y - min_values[current_graph_id]))/maxs[current_graph_id]))+10-20, (instructions[current_graph_id].annotations.yaxis[i].label.text)).attr({
									fill : instructions[current_graph_id].annotations.yaxis[i].label.style.color,
								});
							}
						}
					...
					if((instructions[current_graph_id].annotations.yaxis[i].y1 && instructions[current_graph_id].annotations.yaxis[i].y2) || (instructions[current_graph_id].annotations.yaxis[i].y1 == 0 || instructions[current_graph_id].annotations.yaxis[i].y2 == 0)){
						...
						elems["paper_"+current_graph_id].rect((2*paddingx[current_graph_id]),  (( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(y2 - min_values[current_graph_id]))/maxs[current_graph_id])), width - (4*paddingx[current_graph_id]) , ((height-(4*paddingy[current_graph_id]))*(y2 -y1))/maxs[current_graph_id]).attr({
							stroke : instructions[current_graph_id].annotations.yaxis[i].borderColor,
						...
						if((y2-min_values[current_graph_id]) > maxs[current_graph_id]/2){
							elems["paper_"+current_graph_id].rect((2*paddingx[current_graph_id]) + (i*50+50), (( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(y2 - min_values[current_graph_id]))/maxs[current_graph_id])), (instructions[current_graph_id].annotations.yaxis[i].label.text.length*7)+20, 20).attr({
								fill : instructions[current_graph_id].annotations.yaxis[i].label.style.background,
								stroke : instructions[current_graph_id].annotations.yaxis[i].label.borderColor,
							});
							elems["paper_"+current_graph_id].text((2*paddingx[current_graph_id]) + (i*50+50) + ((instructions[current_graph_id].annotations.yaxis[i].label.text.length*7)+20)/2, (( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(y2 - min_values[current_graph_id]))/maxs[current_graph_id]))+10, (instructions[current_graph_id].annotations.yaxis[i].label.text)).attr({
								fill : instructions[current_graph_id].annotations.yaxis[i].label.style.color,
							});
							
						}else{
							elems["paper_"+current_graph_id].rect((2*paddingx[current_graph_id]) + (i*50+50), (( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(y2 - min_values[current_graph_id]))/maxs[current_graph_id]))-20, (instructions[current_graph_id].annotations.yaxis[i].label.text.length*7)+20, 20).attr({
								fill : instructions[current_graph_id].annotations.yaxis[i].label.style.background,
								stroke : instructions[current_graph_id].annotations.yaxis[i].label.borderColor,
							});
							elems["paper_"+current_graph_id].text((2*paddingx[current_graph_id]) + (i*50+50) + ((instructions[current_graph_id].annotations.yaxis[i].label.text.length*7)+20)/2, ( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(y2 - min_values[current_graph_id]))/maxs[current_graph_id])-10, (instructions[current_graph_id].annotations.yaxis[i].label.text)).attr({
						...
						for(var j = 0; j < datas[current_graph_id].length ; j ++){
							if(datas[current_graph_id][j].month == instructions[current_graph_id].annotations.points[i].x){
								if(instructions[current_graph_id].annotations.points[i].y <= max_values[current_graph_id] && instructions[current_graph_id].annotations.points[i].y >= min_values[current_graph_id]){
									...
									elems["paper_"+current_graph_id].circle(((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))), (height - (2*paddingy[current_graph_id])) - ((parseInt(instructions[current_graph_id].annotations.points[i].y) - min_values[current_graph_id])*((height-(4*paddingy[current_graph_id]))/maxs[current_graph_id])), instructions[current_graph_id].annotations.points[i].marker.radius).attr({
									...
									if(parseInt(instructions[current_graph_id].annotations.points[i].y - min_values[current_graph_id]) > maxs[current_graph_id]/2){
										if(((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))) < 1*((width-(4*paddingx[current_graph_id]))/4)){
											rect_x = ((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1)))  + (instructions[current_graph_id].annotations.points[i].marker.radius + instructions[current_graph_id].annotations.points[i].marker.strokeWidth + 3);
											rect_y = ( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(parseInt(instructions[current_graph_id].annotations.points[i].y - min_values[current_graph_id]))/maxs[current_graph_id]))- 10;
											text_x = ((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))) + ((instructions[current_graph_id].annotations.points[i].label.text.length*7)+20)/2 + (instructions[current_graph_id].annotations.points[i].marker.radius + instructions[current_graph_id].annotations.points[i].marker.strokeWidth + 3);
											text_y = ( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(parseInt(instructions[current_graph_id].annotations.points[i].y - min_values[current_graph_id]))/maxs[current_graph_id])) ;
										}
										else if(((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))) > 3*((width-(4*paddingx[current_graph_id]))/4)){
											rect_x = ((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))) - ((instructions[current_graph_id].annotations.points[i].label.text.length*7)+20) - (instructions[current_graph_id].annotations.points[i].marker.radius + instructions[current_graph_id].annotations.points[i].marker.strokeWidth + 3);
											rect_y = ( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(parseInt(instructions[current_graph_id].annotations.points[i].y - min_values[current_graph_id]))/maxs[current_graph_id])) - 10;
											text_x = ((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))) + ((instructions[current_graph_id].annotations.points[i].label.text.length*7)+20)/2 - ((instructions[current_graph_id].annotations.points[i].label.text.length*7)+20) - (instructions[current_graph_id].annotations.points[i].marker.radius + instructions[current_graph_id].annotations.points[i].marker.strokeWidth + 3);
											text_y = ( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(parseInt(instructions[current_graph_id].annotations.points[i].y - min_values[current_graph_id]))/maxs[current_graph_id]));
										}else {
											rect_x = ((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))) - ((instructions[current_graph_id].annotations.points[i].label.text.length*7)+20)/2;
											rect_y = ( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(parseInt(instructions[current_graph_id].annotations.points[i].y - min_values[current_graph_id]))/maxs[current_graph_id])) + (instructions[current_graph_id].annotations.points[i].marker.radius + instructions[current_graph_id].annotations.points[i].marker.strokeWidth + 3);
											text_x = ((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))) + ((instructions[current_graph_id].annotations.points[i].label.text.length*7)+20)/2 - ((instructions[current_graph_id].annotations.points[i].label.text.length*7)+20)/2;
											text_y = ( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(parseInt(instructions[current_graph_id].annotations.points[i].y - min_values[current_graph_id]))/maxs[current_graph_id]))+10 + (instructions[current_graph_id].annotations.points[i].marker.radius + instructions[current_graph_id].annotations.points[i].marker.strokeWidth + 3);
										}
									}else{
										if(((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))) < 1*((width-(4*paddingx[current_graph_id]))/4)){
											rect_x = ((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1)))  + (instructions[current_graph_id].annotations.points[i].marker.radius + instructions[current_graph_id].annotations.points[i].marker.strokeWidth + 3);
											...
											rect_x = ((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))) - ((instructions[current_graph_id].annotations.points[i].label.text.length*7)+20)/2;
											rect_y = ( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(parseInt(instructions[current_graph_id].annotations.points[i].y - min_values[current_graph_id]))/maxs[current_graph_id])) - 20 - (instructions[current_graph_id].annotations.points[i].marker.radius + instructions[current_graph_id].annotations.points[i].marker.strokeWidth + 3);
											text_x = ((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))) + ((instructions[current_graph_id].annotations.points[i].label.text.length*7)+20)/2 - ((instructions[current_graph_id].annotations.points[i].label.text.length*7)+20)/2;
											text_y = ( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(parseInt(instructions[current_graph_id].annotations.points[i].y - min_values[current_graph_id]))/maxs[current_graph_id])) - 10 - (instructions[current_graph_id].annotations.points[i].marker.radius + instructions[current_graph_id].annotations.points[i].marker.strokeWidth + 3);
										}
									}
		...
		if(instructions[current_graph_id].annotations.max){
			if(instructions[current_graph_id].annotations.max.length > 0){	
				if(max_point_values[current_graph_id] <= max_values[current_graph_id] && max_point_values[current_graph_id] >= min_values[current_graph_id]){
					...
					elems["paper_"+current_graph_id].circle(((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))), (height - (2*paddingy[current_graph_id])) - ((parseInt(max_point_values[current_graph_id]) - min_values[current_graph_id])*((height-(4*paddingy[current_graph_id]))/maxs[current_graph_id])), instructions[current_graph_id].annotations.max[0].marker.radius).attr({
					...
					if(parseInt(max_point_values[current_graph_id] - min_values[current_graph_id]) > maxs[current_graph_id]/2){
						if(((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))) < 1*((width-(4*paddingx[current_graph_id]))/4)){
							rect_x = ((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1)))  + (instructions[current_graph_id].annotations.max[0].marker.radius + instructions[current_graph_id].annotations.max[0].marker.strokeWidth + 3);
							rect_y = ( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(parseInt(max_point_values[current_graph_id] - min_values[current_graph_id]))/maxs[current_graph_id]))- 10;
							text_x = ((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))) + ((instructions[current_graph_id].annotations.max[0].label.text.length*7)+20)/2 + (instructions[current_graph_id].annotations.max[0].marker.radius + instructions[current_graph_id].annotations.max[0].marker.strokeWidth + 3);
							text_y = ( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(parseInt(max_point_values[current_graph_id] - min_values[current_graph_id]))/maxs[current_graph_id])) ;
						}
						else if(((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))) > 3*((width-(4*paddingx[current_graph_id]))/4)){
							rect_x = ((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))) - ((instructions[current_graph_id].annotations.max[0].label.text.length*7)+20) - (instructions[current_graph_id].annotations.max[0].marker.radius + instructions[current_graph_id].annotations.max[0].marker.strokeWidth + 3);
							rect_y = ( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(parseInt(max_point_values[current_graph_id] - min_values[current_graph_id]))/maxs[current_graph_id])) - 10;
							text_x = ((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))) + ((instructions[current_graph_id].annotations.max[0].label.text.length*7)+20)/2 - ((instructions[current_graph_id].annotations.max[0].label.text.length*7)+20) - (instructions[current_graph_id].annotations.max[0].marker.radius + instructions[current_graph_id].annotations.max[0].marker.strokeWidth + 3);
							text_y = ( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(parseInt(max_point_values[current_graph_id] - min_values[current_graph_id]))/maxs[current_graph_id]));
						}else {
							rect_x = ((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))) - ((instructions[current_graph_id].annotations.max[0].label.text.length*7)+20)/2;
							rect_y = ( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(parseInt(max_point_values[current_graph_id] - min_values[current_graph_id]))/maxs[current_graph_id])) + (instructions[current_graph_id].annotations.max[0].marker.radius + instructions[current_graph_id].annotations.max[0].marker.strokeWidth + 3);
							text_x = ((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))) + ((instructions[current_graph_id].annotations.max[0].label.text.length*7)+20)/2 - ((instructions[current_graph_id].annotations.max[0].label.text.length*7)+20)/2;
							text_y = ( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(parseInt(max_point_values[current_graph_id] - min_values[current_graph_id]))/maxs[current_graph_id]))+10 + (instructions[current_graph_id].annotations.max[0].marker.radius + instructions[current_graph_id].annotations.max[0].marker.strokeWidth + 3);
						}
					}else{
						if(((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))) < 1*((width-(4*paddingx[current_graph_id]))/4)){
							rect_x = ((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1)))  + (instructions[current_graph_id].annotations.max[0].marker.radius + instructions[current_graph_id].annotations.max[0].marker.strokeWidth + 3);
							...
							rect_x = ((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))) - ((instructions[current_graph_id].annotations.max[0].label.text.length*7)+20)/2;
							rect_y = ( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(parseInt(max_point_values[current_graph_id] - min_values[current_graph_id]))/maxs[current_graph_id])) - 20 - (instructions[current_graph_id].annotations.max[0].marker.radius + instructions[current_graph_id].annotations.max[0].marker.strokeWidth + 3);
							text_x = ((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))) + ((instructions[current_graph_id].annotations.max[0].label.text.length*7)+20)/2 - ((instructions[current_graph_id].annotations.max[0].label.text.length*7)+20)/2;
							text_y = ( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(parseInt(max_point_values[current_graph_id] - min_values[current_graph_id]))/maxs[current_graph_id])) - 10 - (instructions[current_graph_id].annotations.max[0].marker.radius + instructions[current_graph_id].annotations.max[0].marker.strokeWidth + 3);
						}
					}
		...
		if(instructions[current_graph_id].annotations.min){
			if(instructions[current_graph_id].annotations.min.length > 0){	
				if(min_point_values[current_graph_id] <= max_values[current_graph_id] && min_point_values[current_graph_id] >= min_values[current_graph_id]){
					...
					elems["paper_"+current_graph_id].circle(((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))), (height - (2*paddingy[current_graph_id])) - ((parseInt(min_point_values[current_graph_id]) - min_values[current_graph_id])*((height-(4*paddingy[current_graph_id]))/maxs[current_graph_id])), instructions[current_graph_id].annotations.min[0].marker.radius).attr({
					...
					if(parseInt(min_point_values[current_graph_id] - min_values[current_graph_id]) > maxs[current_graph_id]/2){
						if(((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))) < 1*((width-(4*paddingx[current_graph_id]))/4)){
							rect_x = ((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1)))  + (instructions[current_graph_id].annotations.min[0].marker.radius + instructions[current_graph_id].annotations.min[0].marker.strokeWidth + 3);
							rect_y = ( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(parseInt(min_point_values[current_graph_id] - min_values[current_graph_id]))/maxs[current_graph_id]))- 10;
							text_x = ((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))) + ((instructions[current_graph_id].annotations.min[0].label.text.length*7)+20)/2 + (instructions[current_graph_id].annotations.min[0].marker.radius + instructions[current_graph_id].annotations.min[0].marker.strokeWidth + 3);
							text_y = ( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(parseInt(min_point_values[current_graph_id] - min_values[current_graph_id]))/maxs[current_graph_id])) ;
						}
						else if(((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))) > 3*((width-(4*paddingx[current_graph_id]))/4)){
							rect_x = ((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))) - ((instructions[current_graph_id].annotations.min[0].label.text.length*7)+20) - (instructions[current_graph_id].annotations.min[0].marker.radius + instructions[current_graph_id].annotations.min[0].marker.strokeWidth + 3);
							rect_y = ( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(parseInt(min_point_values[current_graph_id] - min_values[current_graph_id]))/maxs[current_graph_id])) - 10;
							text_x = ((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))) + ((instructions[current_graph_id].annotations.min[0].label.text.length*7)+20)/2 - ((instructions[current_graph_id].annotations.min[0].label.text.length*7)+20) - (instructions[current_graph_id].annotations.min[0].marker.radius + instructions[current_graph_id].annotations.min[0].marker.strokeWidth + 3);
							text_y = ( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(parseInt(min_point_values[current_graph_id] - min_values[current_graph_id]))/maxs[current_graph_id]));
						}else {
							rect_x = ((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))) - ((instructions[current_graph_id].annotations.min[0].label.text.length*7)+20)/2;
							rect_y = ( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(parseInt(min_point_values[current_graph_id] - min_values[current_graph_id]))/maxs[current_graph_id])) + (instructions[current_graph_id].annotations.min[0].marker.radius + instructions[current_graph_id].annotations.min[0].marker.strokeWidth + 3);
							text_x = ((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))) + ((instructions[current_graph_id].annotations.min[0].label.text.length*7)+20)/2 - ((instructions[current_graph_id].annotations.min[0].label.text.length*7)+20)/2;
							text_y = ( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(parseInt(min_point_values[current_graph_id] - min_values[current_graph_id]))/maxs[current_graph_id]))+10 + (instructions[current_graph_id].annotations.min[0].marker.radius + instructions[current_graph_id].annotations.min[0].marker.strokeWidth + 3);
						}
					}else{
						if(((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))) < 1*((width-(4*paddingx[current_graph_id]))/4)){
							rect_x = ((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1)))  + (instructions[current_graph_id].annotations.min[0].marker.radius + instructions[current_graph_id].annotations.min[0].marker.strokeWidth + 3);
							...
							rect_x = ((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))) - ((instructions[current_graph_id].annotations.min[0].label.text.length*7)+20)/2;
							rect_y = ( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(parseInt(min_point_values[current_graph_id] - min_values[current_graph_id]))/maxs[current_graph_id])) - 20 - (instructions[current_graph_id].annotations.min[0].marker.radius + instructions[current_graph_id].annotations.min[0].marker.strokeWidth + 3);
							text_x = ((2*paddingx[current_graph_id])+j*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))) + ((instructions[current_graph_id].annotations.min[0].label.text.length*7)+20)/2 - ((instructions[current_graph_id].annotations.min[0].label.text.length*7)+20)/2;
							text_y = ( (height-(2*paddingy[current_graph_id])) - ((height-(4*paddingy[current_graph_id]))*(parseInt(min_point_values[current_graph_id] - min_values[current_graph_id]))/maxs[current_graph_id])) - 10 - (instructions[current_graph_id].annotations.min[0].marker.radius + instructions[current_graph_id].annotations.min[0].marker.strokeWidth + 3);
						}
					}
...
function x_axis_hover_design(){
	if(instructions[current_graph_id].x_axis_hover_design == true){
		for(var i=0; i<1; i++){
			var  p = "M "+ 2*paddingx[current_graph_id] +" " + 2*paddingy[current_graph_id] + " l 0 "+  (height-(4*paddingy[current_graph_id])) +" 0";
			var path1 = elems["paper_"+current_graph_id].path(p).attr({
				'stroke-width' : .5,
				//"stroke-dasharray": " - ",
				stroke : '#949494',
...
function y_axis_hover_design(){
	if(instructions[current_graph_id].y_axis_hover_design == true){
		for(var i=0; i<instructions[current_graph_id].path_no; i++){
			var  p = "M "+ 2*paddingx[current_graph_id] +" " + ( (height-(2*paddingy[current_graph_id]))-((height-(4*paddingy[current_graph_id]))/(y_division_nos[current_graph_id]-1))*i ) + " l "+  (width-(4*paddingx[current_graph_id])) +" 0";
			var path1 = elems["paper_"+current_graph_id].path(p).attr({
				'stroke-width' : .5,
				//"stroke-dasharray": " - ",
				stroke : '#949494',
...
function popup_design(){	
	var pop_rect = elems["paper_"+current_graph_id].rect(0, (height - (4*paddingy[current_graph_id])), 0, instructions[current_graph_id].path_no*30, 2).attr({
		fill : '#fcfcfc',//'#f2f4ff',
	...
	for(var i=0; i<instructions[current_graph_id].path_no; i++){
		var text1 = elems["paper_"+current_graph_id].text(20, /*(15 + (30*i))*/(height - (4*paddingy[current_graph_id])), "").attr({
			fill : path_details[current_graph_id][i].color,
			'font-size' : '13px',
			'text-anchor' : 'start',
			'font-weight' : 'bold',
...
function popup_footer_design(){
	//for rect
	var rect1 = elems["paper_"+current_graph_id].rect(2*paddingx[current_graph_id], height - (2*paddingy[current_graph_id]), 0, 40, 5).attr({
		fill : '#f1f1f1',
		stroke : '#b6b6b6',
		opacity : 0,
	});
	...
	//for text
	var text1 = elems["paper_"+current_graph_id].text(2*paddingx[current_graph_id], height - (1.5*paddingy[current_graph_id]), "2019").attr({
		'font-size' : '13px',
			'font-weight' : 'bold',
		opacity : 0,
	});
...
function collect_popup_point(){
	...
	var popup_point_y = [];
	for(var i = 0; i < datas[current_graph_id].length; i++){
		popup_point_x.push(((2*paddingx[current_graph_id])+i*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))));
		popup_point_y.push((height - (2*paddingy[current_graph_id])) - ((Math.max(...Object.values(datas[current_graph_id][i].value))  - min_values[current_graph_id])*((height-(4*paddingy[current_graph_id]))/maxs[current_graph_id])));
	}
...
function draw_popup_rect(){
	for(var i = 0; i< datas[current_graph_id].length; i++){
		var rect = elems["paper_"+current_graph_id].rect((((2*paddingx[current_graph_id])-(((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))/2))+i*((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1))),2*paddingy[current_graph_id],((width-(4*paddingx[current_graph_id]))/(datas[current_graph_id].length-1)),(height - (4*paddingy[current_graph_id])));
		rect.attr({
			fill : "transparent",
			opacity : 0,
		});
...
function x_axis_hover(id){
	...
	if(instructions[graph_id].x_axis_hover_design == true){
		for(var i = 0; i<1; i++){
			var  p  = "M "+ ((2*paddingx[current_graph_id])+id*((width-(4*paddingx[current_graph_id]))/(datas[graph_id].length-1))) +" " + (2*paddingy[current_graph_id]) + " l 0 "+  ( (height - (4*paddingy[current_graph_id])) ) ;
...
function y_axis_hover(id){
	...
	if(instructions[graph_id].y_axis_hover_design == true){
		for(var i = 0; i<parseInt(instructions[graph_id].path_no); i++){
			var  p  = "M "+ 2*paddingx[current_graph_id] +" " + ( (height - (2*paddingy[current_graph_id])) - ((Object.values(datas[graph_id][id].value)[i] - min_values[graph_id])*((height-(4*paddingy[current_graph_id]))/maxs[graph_id])) ) + " l "+  (width-(4*paddingx[current_graph_id])) +" 0";
...
function select_area(){
	var rect1 = elems["paper_"+current_graph_id].rect(((2*paddingx[current_graph_id])), 2*paddingy[current_graph_id] , 0, (height-(4*paddingy[current_graph_id]))).attr({
		opacity : 0,
	});
...
function mousedown(e){
	...
	if(instructions[graph_id].select_area == true){
		...
		elems['zoom_rect_'+graph_id].attr({
			x : ((2*paddingx[current_graph_id])+id*((width-(4*paddingx[current_graph_id]))/(datas[graph_id].length-1))),
			y :  2*paddingy[current_graph_id],
			'stroke-width' : 2,
			stroke : '#00376e',
			fill : '#0068d0',
			opacity : .1,
		...
		val1 = ((2*paddingx[current_graph_id])+id*((width-(4*paddingx[current_graph_id]))/(datas[graph_id].length-1)));
	}
